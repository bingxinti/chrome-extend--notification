...
function onClickHandler(info, tab) {
    ...
    } else if (info.menuItemId == "jsqr-linksInSelection") {
        data = [];
        info.selectionText.replace(/(http[^\s]+)/g,function($1){
        	data.push($1);
        });
        data = data.join(',');
...
chrome.runtime.onInstalled.addListener(function() {
    ...
    var contextsIDs = ["page", "selection", "link", "image", "linksInSelection"];
    var contexts = ["page", "selection", "link", "image", "selection"];
    ...
    chrome.i18n.getMessage("context_menu_image"), '所选文本中的链接'];
